...
const MapLocationInput: React.FC<MapLocationInputProps> = ({ 
      ...
      map.on('click', (e: any) => {
        if (drawing) {
          ...
          if (newPoints.length >= 3) {
            ...
            // Calculate centroid
            const centroid = calculateCentroid(newPoints.map(p => ({ lat: p[0], lng: p[1] })));
  ...
  }, [mapLoaded, drawing, points, setLatitude, setLongitude, useManualInput]);
    ...
    <div className="space-y-4">
      ...
      {drawing && (
        <div className="p-3 bg-blue-50 rounded-lg text-sm text-blue-700">
          Click on the map to mark your farm boundaries. You need at least 3 points.
      ...
      {(latitude && longitude) && (
        <div className="p-3 bg-green-50 rounded-lg">
          <p className="text-sm text-green-700">
            ✓ Farm center coordinates: {parseFloat(latitude).toFixed(6)}, {parseFloat(longitude).toFixed(6)}
          </p>
        </div>
      )}
    </div>
