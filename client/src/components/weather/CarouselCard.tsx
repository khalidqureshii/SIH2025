...
//           `${LINK}/api/utils/translate/content=${dayData.condition.text}&lang=${lang}`
//         );
...
//             </>
//           ) : (
//             // Replace "Hello" with your actual text once loaded
//             "Hello"
//             // or {dayData.condition.text}
//           )}
//         </p>
...
const CarouselCard: React.FC<CarouselCardProps> = (props) => {
  ...
  useEffect(() => {
    const fetchCondition = async () => {
      try {
        if (lang === "en") {
          // Directly use English text
          setConditionText(dayData.condition.text);
          setLoadingText(false);
        } else {
          // Send as POST body instead of query params
          const response = await fetch(`${LINK}/api/utils/translate`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              content: dayData.condition.text,
              language: lang,
            }),
          });
          ...
          // Assuming your API returns { translatedText: "..." }
          console.log("Fetched");
          console.log(data);
          setConditionText(data.translated || dayData.condition.text);
          setLoadingText(false);
        }
      } catch (error) {
        console.error("Error fetching translation:", error);
        setConditionText(dayData.condition.text); // fallback to English
        setLoadingText(false);
      }
    };
