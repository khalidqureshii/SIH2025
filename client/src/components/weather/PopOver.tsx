...
const PopOver: React.FC<PopoverProps> = (props) => {
  ...
  const formatDateLabels = (dateStr: string) => {
    ...
    let dayNames: string[] = t(`popover.weekdays.long`, {
      returnObjects: true,
    }) as string[];
    let monthNames: string[] = t(`popover.months.short`, {
      returnObjects: true,
    }) as string[];

    if (!dayNames || !dayNames.length) {
      dayNames = t(`popover.weekdays.long`, {
        lng: "en",
        returnObjects: true,
      }) as string[];
    }
    if (!monthNames || !monthNames.length) {
      monthNames = t(`popover.months.short`, {
        lng: "en",
        returnObjects: true,
      }) as string[];
    }
