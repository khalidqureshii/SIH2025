...
const Timeline: React.FC<TimelineProps> = ({ forecast, loading, error }) => {
  ...
  return (
    <div className="relative flex justify-center items-center w-full">
      <div className="flex flex-nowrap overflow-x-auto gap-4 px-4 py-6 scroll-smooth snap-x snap-mandatory scrollbar-hide">
        {forecast.map((day, index) => {
          const risk =
            day.day.maxwind_kph > 40
              ? t("timeline.risk_labels.high_winds")
          ...
          return (
            <div
              key={day.date}
              className="flex-shrink-0 snap-center min-w-[160px] max-w-[200px] p-4 rounded-2xl shadow-md bg-white text-center"
            >
              <p className="text-sm font-semibold text-gray-700">
                {formatDayLabel(day.date, index)}
              </p>
