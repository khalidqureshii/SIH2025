...
const stateKeys: string[] = [
  "scheme.states.all",
  "scheme.states.andhraPradesh",
  "scheme.states.arunachalPradesh",
  "scheme.states.assam",
  "scheme.states.bihar",
  "scheme.states.chhattisgarh",
  "scheme.states.goa",
  "scheme.states.gujarat",
  "scheme.states.haryana",
  "scheme.states.himachalPradesh",
  "scheme.states.jharkhand",
  "scheme.states.karnataka",
  "scheme.states.kerala",
  "scheme.states.madhyaPradesh",
  "scheme.states.maharashtra",
  "scheme.states.manipur",
  "scheme.states.meghalaya",
  "scheme.states.mizoram",
  "scheme.states.nagaland",
  "scheme.states.odisha",
  "scheme.states.punjab",
  "scheme.states.rajasthan",
  "scheme.states.sikkim",
  "scheme.states.tamilNadu",
  "scheme.states.telangana",
  "scheme.states.tripura",
  "scheme.states.uttarPradesh",
  "scheme.states.uttarakhand",
  "scheme.states.westBengal",
  "scheme.states.delhi",
  "scheme.states.jammuKashmir",
  "scheme.states.ladakh",
  "scheme.states.puducherry",
  "scheme.states.chandigarh",
  "scheme.states.andamanNicobar",
  "scheme.states.dadraNagarHaveli",
  "scheme.states.damanDiu",
];

const stateApiMap: Record<string, string> = {
  "scheme.states.all": "All",
  "scheme.states.andhraPradesh": "Andhra Pradesh",
  "scheme.states.arunachalPradesh": "Arunachal Pradesh",
  "scheme.states.assam": "Assam",
  "scheme.states.bihar": "Bihar",
  "scheme.states.chhattisgarh": "Chhattisgarh",
  "scheme.states.goa": "Goa",
  "scheme.states.gujarat": "Gujarat",
  "scheme.states.haryana": "Haryana",
  "scheme.states.himachalPradesh": "Himachal Pradesh",
  "scheme.states.jharkhand": "Jharkhand",
  "scheme.states.karnataka": "Karnataka",
  "scheme.states.kerala": "Kerala",
  "scheme.states.madhyaPradesh": "Madhya Pradesh",
  "scheme.states.maharashtra": "Maharashtra",
  "scheme.states.manipur": "Manipur",
  "scheme.states.meghalaya": "Meghalaya",
  "scheme.states.mizoram": "Mizoram",
  "scheme.states.nagaland": "Nagaland",
  "scheme.states.odisha": "Odisha",
  "scheme.states.punjab": "Punjab",
  "scheme.states.rajasthan": "Rajasthan",
  "scheme.states.sikkim": "Sikkim",
  "scheme.states.tamilNadu": "Tamil Nadu",
  "scheme.states.telangana": "Telangana",
  "scheme.states.tripura": "Tripura",
  "scheme.states.uttarPradesh": "Uttar Pradesh",
  "scheme.states.uttarakhand": "Uttarakhand",
  "scheme.states.westBengal": "West Bengal",
  "scheme.states.delhi": "Delhi",
  "scheme.states.jammuKashmir": "Jammu & Kashmir",
  "scheme.states.ladakh": "Ladakh",
  "scheme.states.puducherry": "Puducherry",
  "scheme.states.chandigarh": "Chandigarh",
  "scheme.states.andamanNicobar": "Andaman & Nicobar",
  "scheme.states.dadraNagarHaveli": "Dadra & Nagar Haveli",
  "scheme.states.damanDiu": "Daman & Diu",
};
...
// --- MAIN COMPONENT ---
export default function Scheme() {
  const [selectedState, setSelectedState] = useState<string>("scheme.states.all");
  const [schemes, setSchemes] = useState<Scheme[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const { t, i18n } = useTranslation();
  const lang = i18n.language;
  ...
  useEffect(() => {
    const fetchSchemes = async (stateKey: string) => {
      ...
      try {
        const apiValue = stateApiMap[stateKey] || "All";
        const res = await fetch(`${LINK}/api/scheme/getScheme?state=${apiValue}&language=${lang}`);
        if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
        const json = await res.json();
        const schemesData: Scheme[] = json.schemes.map((item: any) => item);
        setSchemes(schemesData);
    ...
    <div className="min-h-screen overflow-x-hidden relative">
      <div className="relative container mx-auto px-4 md:px-6 py-12">
        ...
        <div className="bg-white rounded-3xl shadow-xl p-6 md:p-8 mb-12 border border-gray-100 flex flex-col md:flex-row justify-between items-stretch gap-6">
          ...
          {/* Labeled Dropdown with Original Height */}
          <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-2xl w-full md:w-auto">
            <label className="text-gray-700 font-semibold whitespace-nowrap px-2">
              {t("scheme.filters.label")}
            </label>
            <select
              value={selectedState}
              onChange={(e) => setSelectedState(e.target.value)}
              className="w-full md:w-auto px-4 py-2 bg-white rounded-xl border-2 border-gray-200 shadow-sm focus:ring-4 focus:ring-green-100 focus:border-green-400 focus:outline-none transition-all duration-200 font-medium text-gray-700"
              // style={{ height: "44px" }} // match original dropdown height
            >
              {stateKeys.map((key) => (
                <option key={key} value={key}>
                  {t(key)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {schemes.map((scheme) => (
              ...
              >
                <div className="flex items-center gap-3 mb-4">
                  <div className="p-3 bg-green-100 rounded-2xl text-green-700 text-2xl">
                    {scheme.schemeCategory && scheme.schemeCategory.length > 0
                      ? categoryIcons[scheme.schemeCategory[0]] || <FaLayerGroup />
                      : <FaLayerGroup />}
                  </div>
                  <div
                    className={`inline-block px-3 py-1 rounded-full text-xs font-bold border ${getLevelColor(
                ...
                <h2 className="text-2xl font-bold text-gray-900 mb-2">{scheme.schemeName}</h2>
                <p className="text-gray-700 mb-3">{scheme.briefDescription}</p>
