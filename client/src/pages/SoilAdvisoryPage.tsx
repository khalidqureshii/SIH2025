...
interface SensorData {
  ...
  moisture: number | null;       // for sliders/UI
  soilMoisture: number | null;   // for backend
  ph: number | null;             // for sliders/UI
  pH: number | null;             // for backend
  n: number | null;
  p: number | null;
  k: number | null;
  nvdi: number | null;
...
export default function Alternate() {
  ...
  const handleSensorChange = (
    ...
    setSensorData((prev) => {
      ...
      // 🔗 Sync UI <-> Backend keys
      if (field === "ph") updated.pH = num;
      if (field === "pH") updated.ph = num;
      if (field === "moisture") updated.soilMoisture = num;
      if (field === "soilMoisture") updated.moisture = num;
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (useSensor) {
          ...
          if (sensorData.moisture === null) sensorData.moisture = backendResult.satellite_data.soil_moisture;
        ...
        // ... keep your existing result mapping here ...
        setResult({
          weather: {
          ...
          advanced_insights: Array.isArray(backendResult.advanced_insights)
            ? backendResult.advanced_insights
            : [backendResult.advanced_insights],
